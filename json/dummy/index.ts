import { IEachRelatedProductProps } from "@/types/common.type";
import assest from "../assest";

export const eachPowerFullElements = [
  {
    image: assest?.powerfull_image1,
    title: "Space saver",
    description:
      "Compact & sleek that fits everywhere you want. A compact alternative for bulky desktop set-ups rendering the perfect way to maximise your desk space."
  },
  {
    image: assest?.powerfull_image2,
    title: "Fanless",
    description:
      "Compact & sleek that fits everywhere you want. A compact alternative for bulky desktop set-ups rendering the perfect way to maximise your desk space."
  },
  {
    image: assest?.powerfull_image1,
    title: "Space saver",
    description:
      "Compact & sleek that fits everywhere you want. A compact alternative for bulky desktop set-ups rendering the perfect way to maximise your desk space."
  },
  {
    image: assest?.powerfull_image2,
    title: "Fanless",
    description:
      "Compact & sleek that fits everywhere you want. A compact alternative for bulky desktop set-ups rendering the perfect way to maximise your desk space."
  }
];

export const eachSmartElements = [
  {
    image: assest?.smart_img1,
    title: "MSDA",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  },
  {
    image: assest?.smart_img2,
    title: "Fanless",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  },
  {
    image: assest?.smart_img3,
    title: "MSDA",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  },
  {
    image: assest?.smart_img1,
    title: "MSDA",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  },
  {
    image: assest?.smart_img2,
    title: "Fanless",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  },
  {
    image: assest?.smart_img3,
    title: "MSDA",
    description:
      "Maximize your workspace with citrus the ultimate in thin client computing"
  }
];

export const clientsData = [
  {
    img: assest?.cl1
  },
  {
    img: assest?.cl2
  },
  {
    img: assest?.cl3
  },
  {
    img: assest?.cl4
  },
  {
    img: assest?.cl5
  },
  {
    img: assest?.cl6
  },
  {
    img: assest?.cl7
  },
  {
    img: assest?.cl8
  },
  {
    img: assest?.cl9
  },
  {
    img: assest?.cl10
  },
  {
    img: assest?.cl11
  },
  {
    img: assest?.cl12
  }
];

export const updatedDataList = [
  {
    image: assest?.updated_img1,
    publishDate: "Mar 21, 2025",
    title:
      "Unleash The Future Of Performance With 12th Gen Intel® Core™ Mobile Processors",
    redirectUrl: "/"
  },
  {
    image: assest?.updated_img2,
    publishDate: "Mar 21, 2025",
    title: "How Do You Choose The Right Gaming PC?",
    redirectUrl: "/"
  },
  {
    image: assest?.updated_img3,
    publishDate: "Mar 21, 2025",
    title: "Nvidia GeForce RTX 3070 Ti",
    redirectUrl: "/"
  },
  {
    image: assest?.updated_img1,
    publishDate: "Mar 21, 2025",
    title:
      "Unleash The Future Of Performance With 12th Gen Intel® Core™ Mobile Processors",
    redirectUrl: "/"
  },
  {
    image: assest?.updated_img2,
    publishDate: "Mar 21, 2025",
    title: "How Do You Choose The Right Gaming PC?",
    redirectUrl: "/"
  },
  {
    image: assest?.updated_img3,
    publishDate: "Mar 21, 2025",
    title: "Nvidia GeForce RTX 3070 Ti",
    redirectUrl: "/"
  }
];

export const productList: IEachRelatedProductProps[] = [
  {
    title: "EON 13ProX",
    image: assest?.product1,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  },
  {
    title: "UVA Plus LOH610",
    image: assest?.product2,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  },
  {
    title: "RIG Plus Rugged PC",
    image: assest?.product3,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  }
];

export const productList2: IEachRelatedProductProps[] = [
  {
    title: "EON Slim E",
    image: assest?.product4,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  },
  {
    title: "EON Slim N3350",
    image: assest?.product5,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  },
  {
    title: "EON 14Pro",
    image: assest?.product6,
    memory: "Supports upto 64GB 3200 MHz",
    os: "Windows 11 Pro/IoT, Linux",
    processor: " Intel Core 13th Gen. i3 1315U/ i5 1335U/ i7 1355U",
    storage: "M.2 NVMe + 2.5″ SATA",
    route: ""
  }
];

export const resultListData = [
  "Improved FIDS operational potential with almost zero downtime.",
  "Feasible management of Multiple Displays with a single media player.",
  "Yielded increased revenue for advertisers with greater visibility.",
  "Smooth passenger transit due to clear information display in international airports like Mumbai & Delhi."
];

export const successStoryListData = [
  "Citrus Thin clients & technicians not only outperformed other competitive brands but also assisted them for system implementation.",
  "They could stop all unauthorized software & hardware use and access to malicious agents.",
  "Had an advantage of fewer cables, greater security, lower power consumption and backup via a switch which was very important."
];

export const industrialListData = [
  "Citrus Industrial rugged PC was successful in such harsh",
  "They could implement certain software in those PCs which gave them improved data, control in their existing system.",
  "Productive work enhancement due to new Technology and better quality outcome of product.",
  "Better Product gave them better market access. Followed by More Revenue generation for the company.",
  "Had an advantage of fewer cables, greater security, lower power consumption and backup via a switch which was very important."
];
